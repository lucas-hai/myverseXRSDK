...
using static MyVerseXRSDK.GameObjectPoolModule;
#if UNITY_EDITOR
using UnityEditor;
#endif
namespace MyVerseXRSDK
{
    ...
    public static class PoolSystem
    {
        ...
        public static void Init()
        {
            ...
            poolRootTransform.SetParent(MVXRSDK.MVXRSDKManager);
            GameObjectPoolModule.Init(poolRootTransform);
        }

        /// <summary>
        /// SDK UnInit 末尾调用：销毁 PoolRoot GO + null 三个静态字段，
        /// 让下次 Init 可以干净重建。必须在 ClearAll() 之后调用（先清池子内容再释放容器）。
        /// </summary>
        internal static void ResetForUnInit()
        {
            if (poolRootTransform != null)
            {
                // EditMode 测试上下文里 Destroy 是 deferred 的——用 isPlaying 切换确保两种环境都立即生效
                if (UnityEngine.Application.isPlaying)
                    UnityEngine.Object.Destroy(poolRootTransform.gameObject);
                else
                    UnityEngine.Object.DestroyImmediate(poolRootTransform.gameObject);
        ...
        public static bool PushGameObject(string keyName, GameObject obj)
        {
            ...
            {
                MVXRSDKLog.Error("您正在将Null放置对象池");
                return false;
