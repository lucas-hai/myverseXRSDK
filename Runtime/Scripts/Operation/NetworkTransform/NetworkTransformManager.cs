...
namespace MyVerseXRSDK
{
    ...
    internal class NetworkTransformManager
    {
        ...
        private static float                      s_SameRoomDistance = MVXRSDKConfig.NORMAL_DISTANCE;  // 同房间虚影显示距离（米），外部可配，默认 2m
        ...
        public static void UnInitSDK()
        {
            ...
            s_SameRoomDistance = MVXRSDKConfig.NORMAL_DISTANCE;  // 同房间显示距离复位默认 2m
            s_Initialized = false;
        }
        ...
        public static float SameRoomDistance => s_SameRoomDistance;

        /// <summary>
        /// 设置是否同步"同房间（本房间）其他玩家虚影"，并指定其显示距离。
        /// - 开启：后续收到本房间位置推送即创建虚影；显示距离取 displayDistanceMeters（米，&lt;=0 回退默认 2m）。
        /// - 关闭：立即回收已创建的本房间虚影（不影响非本房间的虚影）；距离参数忽略。
        /// 其他房间虚影显示距离固定 2m，不受此参数影响；本机自己的虚影一律不显示。
        /// 任何时机皆可调用（Start 前调用也安全，仅置标志）。
        /// </summary>
        public static void SetSyncSameRoomAvatar(bool enable, float displayDistanceMeters = 2f)
        {
            // 距离仅在开启时应用（关闭时保留，避免默认参数把已设值改写）
            if (enable)
                s_SameRoomDistance = displayDistanceMeters > 0f ? displayDistanceMeters : MVXRSDKConfig.NORMAL_DISTANCE;

            if (s_SyncSameRoomAvatar == enable)
            {
                if (enable) MVXRSDKLog.Info($"NetworkTransformManager: 同房间虚影显示距离更新为 {s_SameRoomDistance}m");
                return;
            }
            s_SyncSameRoomAvatar = enable;
            MVXRSDKLog.Info($"NetworkTransformManager: 同房间虚影同步 {(enable ? $"开启（显示距离 {s_SameRoomDistance}m）" : "关闭")}");
        ...
        private static void OnRoomPositionPush(int errorCode, byte[] buffer)
        {
            ...
            // 自己的虚影一律不显示（无论同房间 / 其他房间）：服务端可能把本机位置也广播回来，这里主动滤掉。
            if (devP.DeviceId == MVXRSDK.DeviceId) return;
