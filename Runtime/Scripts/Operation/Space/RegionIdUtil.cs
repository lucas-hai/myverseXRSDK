...
{
    /// <summary>
    /// 地块 id 契约实现。id 形态与远端区域规格 tagId 完全一致：宽x长、小写 x、整数（如 "6x12"）。
    /// 分量顺序依据《开发者开放 API 文档》§四（tagId 首分量为宽 width、次分量为长 height）；
    /// 小写 x 为 2026-07-10 与后端的最终约定（替代文档样例中的大写 X）。
    ...
    public static class RegionIdUtil
    {
        ...
        /// "宽x长"（小写），如 (len:12.35, width:6.18) → "6x12"。
        ...
        /// <summary>解析 id（"宽x长"，兼容大写 X）回长宽数值（编辑器新建条目缺省长宽的回退来源）；格式不符返回 false。</summary>
