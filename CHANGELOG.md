...
### Changed
- **区域地块可视物位姿改用区域对齐计算结果**：位置/旋转不再取地块 local `position/rotation`，改为 `RegionAlignmentCalculator` 算出的**世界位姿**（与场景根节点同一值），使地块落到区域对齐后的物理位置；尺寸仍取地块长×宽。
- **区域地块可视物改为独立常驻对象**：由「挂已注册 XR 偏移节点下、按 XR 注册/注销显隐」改为**独立场景根对象 + `DontDestroyOnLoad`**（跨场景常驻、不随场景卸载删除，区域物理固定故位姿也常驻）。`RegionTileModule` 去除对 XR 偏移节点的依赖（不再订阅 `OnXROffsetNode*`、不再挂父/移父）；显示条件为 `ShowFloor=true` + 命中本地地块 + 已有快照，仅 UnInit 时销毁。副作用：服务端形态若下发 `showFloor=true` 也会建出该对象（无相机不出画面）
...
### Added
- 操作说明文档 `Documentation~/editor-tools-guide.md`：SDK 验证与地块编辑器逐步操作指引——开发者平台凭据获取（创建游戏生成 appId/AccessToken）、验证失败原因对照、地块创建/编辑/上传门禁、运行时匹配速览、常见问题表；手册 §12 挂链接

### Fixed
- Demo 场景地块编辑器"未保存"标记残留复位
...
### Changed
- 地块 id 契约定稿为**小写 x**（与后端约定，替代 API 文档样例的大写形态）：`MakeRuntimeId` 生成 `宽x长` 小写（如 `6x12`），`TryParseId` 兼容大写 X；宽在前的分量顺序不变。3.1.0/3.1.1 期间创建的大写 id 地块条目需重建（或手改 id 为小写）
